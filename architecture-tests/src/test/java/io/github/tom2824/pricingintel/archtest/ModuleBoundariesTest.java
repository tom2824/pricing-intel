 ...
 * Ces règles sont la version vérifiée des ADR 0001 et 0003.
 */
...
class ModuleBoundariesTest {
    ...
    static final ArchRule http_adapter_ignores_its_siblings_and_the_application = noClasses()
            ...
            .should().dependOnClassesThat().resideInAnyPackage(SCRAPER, SINK, BATCH);

    @ArchTest
    static final ArchRule scraper_uses_the_fetcher_port_never_the_http_module = noClasses()
            .that().resideInAPackage(SCRAPER)
            .should().dependOnClassesThat().resideInAnyPackage(HTTP, SINK, BATCH);

    @ArchTest
    static final ArchRule sinks_ignore_their_siblings_and_the_application = noClasses()
            .that().resideInAPackage(SINK)
            .should().dependOnClassesThat().resideInAnyPackage(HTTP, SCRAPER, BATCH);

    @ArchTest
    static final ArchRule only_the_application_module_uses_spring = noClasses()
            .that().resideOutsideOfPackage(BATCH)
